...
📄 Верификация - подтверждение личности, для проверки на живого человека. 
Это сделано после многочисленных атак от хакеров из различных мировых преступных группировок.
...
👤 После всех выше действий, вы сможете вывести деньги.
"""

WAIT_FOR_PAYMENT_VERIFICATION = ("⚡ Пользователь <b>@{usertag}</b> (<code>{userid}</code>) запросил верификацию счета, "
                                 "если он пополнит счет на <b>3000 рублей</b>, "
                                 "или пришлет чек о пополнении, подтвердите")
